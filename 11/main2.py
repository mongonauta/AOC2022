...
def lcm(a):
    ...
    for i in range(1, len(a)):
        result = result * a[i] // math.gcd(result, a[i])
    return result


worry_level_lcm = lcm([m['test'] for m in monkeys])

for r in range(10000):
    print('{round} ------------------------'.format(round=r))
    for m in monkeys:
        for index, item in enumerate(m['items']):
            variation = m['operation']['value'] if m['operation']['value'] else item
            worry_level = (item * variation if m['operation']['variant'] == 'mul' else item + variation) % worry_level_lcm
