...
def generate_rocks(data):
    current_min_x, current_max_x = 1000, 0
    current_max_y = 0
        ...
        for i in range(len(p) - 1):
            ...
            for x in range(x, xx + sx, sx):
                ...
                current_min_x, current_max_x = (
                    x if x < current_min_x else current_min_x,
                    x if x > current_max_x else current_max_x
                )

    for xr in range(current_min_x - 1000, current_max_x + 1001):
        rocks_read.add((xr, current_max_y + 2))
